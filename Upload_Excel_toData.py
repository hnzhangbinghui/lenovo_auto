...
def insert_data():
    ...
    for i in range(1, sheet.nrows):  # 第一行是标题名，对应表中的字段应该是从第二行开始；
        row_data = sheet.row_values(i)  # 获取该行的所有值
        value = (row_data[0], row_data[1], row_data[2], row_data[3], row_data[4], row_data[5], row_data[6], row_data[7],
                 row_data[8], row_data[9], row_data[10], row_data[11], row_data[12])
        print("第", i, "行的值：", value)
        sql = "INSERT INTO world.zhang_lenovo(id,calculation_id,market,incentive_id,incentive_name,pep_element," \
              "target,cdi_month,achievement,achievementpctg,name,name2,name3)VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s," \
              "%s,%s) "
